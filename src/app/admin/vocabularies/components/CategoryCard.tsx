...
import { IconDots, IconEdit, IconFile, IconFolderFilled, IconGripVertical, IconTrash } from "@tabler/icons-react"
      ...
      <div className="card flex items-center gap-3 rounded bg-white px-4 py-2 pe-2">
        <div className="flex flex-1 items-center gap-2">
          ...
          <div className="flex h-8 w-8 items-center justify-center">
            {hasChildren && <IconFolderFilled className="h-5 w-5 text-gray-400" stroke={1.5} />}
            {!hasChildren && <IconFile className="h-5 w-5 text-gray-400" stroke={1.5} />}
          </div>
          <Button
            noStyle
            onPress={() => toggleChilds()}
            className="font-bold text-gray-800 underline-offset-2 hover:text-gray-900 hover:underline"
          >
            {title}
          </Button>
          {hasChildren && <span className="text-sm text-gray-500">{digitsEnToFa(childrenCount)} زیر دسته</span>}
        </div>
        ...
        <div className="ms-8 border-s border-gray-200 ps-2">
          <div className="flex flex-col gap-2">
            {data?.category.children.map((child) => (
              <CategoryCard category={child as Category} vocabularySlug={vocabularySlug} key={child?.id} />
            ))}
          </div>
        </div>
      )}
