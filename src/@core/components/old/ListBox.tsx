...
export const ListBoxContext =
  createContext<ContextValue<ListBoxContextValue<any>, HTMLDivElement>>(null)
const InternalListBoxContext =
  createContext<InternalListBoxContextValue | null>(null)
...
function ListBox<T>(props: ListBoxProps<T>, ref: ForwardedRef<HTMLDivElement>) {
  ...
  if (state) {
    return isHidden ? null : (
      <ListBoxInner state={state} props={props} listBoxRef={ref} />
    )
  }
  ...
  if (isListDroppable && dragAndDropHooks) {
    ...
    let keyboardDelegate =
      ...
      new ListKeyboardDelegate(
        collection,
        selectionManager.disabledBehavior === "selection"
          ? new Set()
...
function Option<T>({ item }: OptionProps<T>) {
  ...
  let { state, shouldFocusOnHover, dragAndDropHooks, dragState, dropState } =
    useContext(InternalListBoxContext)!
  ...
  let { hoverProps, isHovered } = useHover({
    isDisabled:
      shouldFocusOnHover || (!states.allowsSelection && !states.hasAction)
  })
    ...
    <>
      ...
      >
        ...
        >
          ...
          {states.isSelected && <IconCheck className="h-4 w-4" />}
        </Provider>
      </div>
      {dragAndDropHooks?.useDropIndicator &&
...
function ListBoxDropIndicatorWrapper(
  ...
  return (
    <ListBoxtDropIndicatorForwardRef
      {...props}
...
function ListBoxtDropIndicator(
